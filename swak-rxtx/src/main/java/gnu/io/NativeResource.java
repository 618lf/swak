...
public class NativeResource {
	...
	private InputStream locateResource(String name) {
		...
		String file = "";
		if (OSUtil.isOSX()) {
			file = "/META-INF/native/osx/" + name;
		} else if (OSUtil.isWindows()) {
			if (OSUtil.is64Bit()) {
				file = "/META-INF/native/windows/x86_64/" + name;
			} else {
				file = "/META-INF/native/windows/x86_32/" + name;
			}
			...
			if (OSUtil.isARM()) {
				...
				if (OSUtil.is64Bit()) {
					file = "/META-INF/native/linux/x86_64/" + name;
				} else {
					file = "/META-INF/native/linux/x86_32/" + name;
				}
			}
		} else if (OSUtil.isFreeBSD()) {
			if (OSUtil.is64Bit()) {
				file = "/META-INF/native/freebsd/x86_64/" + name;
			} else {
				file = "/META-INF/native/freebsd/x86_32/" + name;
			}
		} else {
			// System.err.println("Can't load native file: "+name+" for os arch:
			// "+OSUtil.getOsArch());
			return null;
		}
