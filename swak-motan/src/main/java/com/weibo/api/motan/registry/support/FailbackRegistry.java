...
public abstract class FailbackRegistry extends AbstractRegistry {
	...
	private static ScheduledExecutorService retryExecutor = Contexts
			.createScheduledContext("Motan.FailedRegisteredRetry", 1, true, 2, TimeUnit.SECONDS);
	static {
		...
		try {
			...
			if (isCheckingUrls(getUrl(), url)) {
				throw new MotanFrameworkException(
						String.format("[%s] false to registery %s to %s", registryClassName, url, getUrl()), e);
			}
	...
	public void unregister(URL url) {
		...
		try {
			...
			if (isCheckingUrls(getUrl(), url)) {
				throw new MotanFrameworkException(
						String.format("[%s] false to unregistery %s to %s", registryClassName, url, getUrl()), e);
			}
	...
	public void subscribe(URL url, NotifyListener listener) {
		...
		try {
			...
			if (cachedUrls != null && cachedUrls.size() > 0) {
				...
				LoggerUtil.warn(String.format("[%s] false to subscribe %s from %s", registryClassName, url, getUrl()),
						e);
				throw new MotanFrameworkException(
						String.format("[%s] false to subscribe %s from %s", registryClassName, url, getUrl()), e);
			}
	...
	public void unsubscribe(URL url, NotifyListener listener) {
		...
		try {
			...
			if (isCheckingUrls(getUrl(), url)) {
				throw new MotanFrameworkException(
						String.format("[%s] false to unsubscribe %s from %s", registryClassName, url, getUrl()), e);
			}
	...
	private void retry() {
		if (!failedRegistered.isEmpty()) {
			...
			try {
				...
				LoggerUtil.warn(
						String.format("[%s] Failed to retry register, retry later, failedRegistered.size=%s, cause=%s",
		...
		if (!failedUnregistered.isEmpty()) {
			...
			try {
				...
				LoggerUtil.warn(String.format(
						"[%s] Failed to retry unregister, retry later, failedUnregistered.size=%s, cause=%s",
		...
		if (!failedSubscribed.isEmpty()) {
			...
			if (failed.size() > 0) {
				...
				try {
					...
					LoggerUtil.warn(String.format(
							"[%s] Failed to retry subscribe, retry later, failedSubscribed.size=%s, cause=%s",
		...
		if (!failedUnsubscribed.isEmpty()) {
			...
			if (failed.size() > 0) {
				...
				try {
					...
					LoggerUtil.warn(String.format(
							"[%s] Failed to retry unsubscribe, retry later, failedUnsubscribed.size=%s, cause=%s",
