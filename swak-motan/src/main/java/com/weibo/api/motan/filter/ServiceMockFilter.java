...
public class ServiceMockFilter implements Filter {
			...
			try {
				...
				for (Method method : methods) {
					if (methodDesc.equals(ReflectUtil.getMethodDesc(method))) {
						Object value = invoke(mockClass.getDeclaredConstructor().newInstance(), method,
								request.getArguments(), info);
	...
	private Object invoke(Object clz, Method method, Object[] args, MockInfo info)
	...
	public static class MockInfo {
		...
		public MockInfo(URL url) {
			...
			errorRate = Double
					.valueOf(url.getParameter(URLParamType.errorRate.getName(), URLParamType.errorRate.getValue()));
