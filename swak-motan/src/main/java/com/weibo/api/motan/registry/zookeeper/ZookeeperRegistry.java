...
public class ZookeeperRegistry extends CommandFailbackRegistry implements Closable {
	...
	protected void subscribeService(final URL url, final ServiceListener serviceListener) {
		try {
			...
			try {
				...
				LoggerUtil.warn("[ZookeeperRegistry] subscribe service: create node error, path=%s, msg=%s",
						ZkUtils.toNodePath(url, ZkNodeType.CLIENT), e.getMessage());
			}
			...
			throw new MotanFrameworkException(
					String.format("Failed to subscribe %s to zookeeper(%s), cause: %s", url, getUrl(), e.getMessage()),
					e);
		} finally {
			clientLock.unlock();
		}
			...
			if (zkDataListener == null) {
				dataChangeListeners.putIfAbsent(commandListener, new IZkDataListener() {
					...
					public void handleDataChange(String dataPath, Object data) throws Exception {
						...
						LoggerUtil.info(String.format("[ZookeeperRegistry] command data change: path=%s, command=%s",
	...
	protected void unsubscribeService(URL url, ServiceListener serviceListener) {
		try {
			...
			throw new MotanFrameworkException(String.format(
					"Failed to unsubscribe service %s to zookeeper(%s), cause: %s", url, getUrl(), e.getMessage()), e);
		} finally {
			clientLock.unlock();
		}
	}
	...
	protected void unsubscribeCommand(URL url, CommandListener commandListener) {
		try {
			...
			throw new MotanFrameworkException(String.format(
					"Failed to unsubscribe command %s to zookeeper(%s), cause: %s", url, getUrl(), e.getMessage()), e);
		} finally {
			clientLock.unlock();
		}
	}
	...
	protected List<URL> discoverService(URL url) {
		try {
			...
			throw new MotanFrameworkException(String.format(
					"Failed to discover service %s from zookeeper(%s), cause: %s", url, getUrl(), e.getMessage()), e);
		}
	}
	...
	protected String discoverCommand(URL url) {
		try {
			...
			throw new MotanFrameworkException(String.format(
					"Failed to discover command %s from zookeeper(%s), cause: %s", url, getUrl(), e.getMessage()));
		}
	}
	...
	protected void doRegister(URL url) {
		try {
			...
			throw new MotanFrameworkException(
					String.format("Failed to register %s to zookeeper(%s), cause: %s", url, getUrl(), e.getMessage()),
					e);
		} finally {
			serverLock.unlock();
		}
	...
	protected void doUnregister(URL url) {
		try {
			...
			throw new MotanFrameworkException(
					String.format("Failed to unregister %s to zookeeper(%s), cause: %s", url, getUrl(), e.getMessage()),
					e);
		} finally {
			serverLock.unlock();
		}
			...
			for (String node : currentChilds) {
				...
				if (StringUtils.isNotBlank(data)) {
					try {
						...
						LoggerUtil.warn(String.format("Found malformed urls from ZookeeperRegistry, path=%s", nodePath),
								e);
					}
				}
				if (newurl == null) {
