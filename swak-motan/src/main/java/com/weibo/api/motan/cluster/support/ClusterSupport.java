...
public class ClusterSupport<T> implements NotifyListener {
	...
	public void init() {
		...
		boolean check = Boolean
				.parseBoolean(url.getParameter(URLParamType.check.getName(), URLParamType.check.getValue()));
		if (!CollectionUtil.isEmpty(cluster.getReferers()) || !check) {
			cluster.init();

			if (CollectionUtil.isEmpty(cluster.getReferers()) && !check) {
				LoggerUtil.warn(String.format("refer:%s", this.url.getPath() + "/" + this.url.getVersion()),
						"No services");
			}
			LoggerUtil.info("cluster init cost " + (System.currentTimeMillis() - start) + ", refer size:"
					+ (cluster.getReferers() == null ? 0 : cluster.getReferers().size()) + ", cluster:"
					+ cluster.getUrl().toSimpleString());
			return;
		}

		throw new MotanFrameworkException(
				String.format("ClusterSupport No service urls for the refer:%s, registries:%s", this.url.getIdentity(),
						registryUrls),
	...
	public void destroy() {
		...
		for (URL ru : registryUrls) {
			try {
				...
				LoggerUtil.warn(String.format("Unregister or unsubscribe false for url (%s), registry= %s", url,
						ru.getIdentity()), e);
			}
	...
	public synchronized void notify(URL registryUrl, List<URL> urls) {
		if (CollectionUtil.isEmpty(urls)) {
			...
			LoggerUtil.warn("ClusterSupport config change notify, urls is empty: registry={} service={} urls=[]",
					registryUrl.getUri(), url.getIdentity());

			return;
		}

		LoggerUtil.info("ClusterSupport config change notify: registry={} service={} urls={}", registryUrl.getUri(),
				url.getIdentity(), getIdentities(urls));
	...
	private void onRegistryEmpty(URL excludeRegistryUrl) {
		...
		if (noMoreOtherRefers) {
			LoggerUtil
					.warn(String.format("Ignore notify for no more referers in this cluster, registry: %s, cluster=%s",
	...
	protected Protocol getDecorateProtocol(String protocolName) {
		...
		if (decorateProtocol == null) {
			protocols.putIfAbsent(protocolName, new ProtocolFilterDecorator(
	...
	private void mergeClientConfigs(URL refererURL) {
		String application = refererURL.getParameter(URLParamType.application.getName(),
				URLParamType.application.getValue());
	...
	private void prepareCluster() {
		...
		String loadbalanceName = url.getParameter(URLParamType.loadbalance.getName(),
				URLParamType.loadbalance.getValue());
		...
		LoadBalance<T> loadBalance = ExtensionLoader.getExtensionLoader(LoadBalance.class)
