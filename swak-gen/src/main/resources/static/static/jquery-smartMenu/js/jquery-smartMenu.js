				...
				$.each(dataMenu, function(i, arr) {
					...
					if ($.isArray(arr)) {
						$.each(arr, function(j, obj) {
							var text = obj.text, htmlMenuLi = "", strTitle = "", rand = Math.random().toString().replace(".", ""),icon = obj.icon||'';
							if (text) {
								...
								if ($.isArray(obj.data) && obj.data.length) {
									...
									htmlMenuLi = '<li class="'+ clKey +'li">' +
										'<a href="javascript:" class="'+ clKey +'a"'+ strTitle +' data-key="'+ rand +'"><i class="fa '+ icon +'"></i>'+ text +'</a>' + 
									'</li>';
								}
			...
			this.oncontextmenu = function(e) {
				...
				if (jqueryMenu) {
					jqueryMenu.css({
						display: "block",
						left: (function(){
							var left = e.clientX + params.offsetX;
							if(left + jqueryMenu.width() > $('body').width()) {
							   return e.clientX - jqueryMenu.width();
							}
							return left
						})(),
						top: (function(){
							var top = e.clientY + st + params.offsetY
							if(top + jqueryMenu.height() > $('body').height()) {
							   return e.clientY - jqueryMenu.height();
							}
							return top;
						})()
					});
