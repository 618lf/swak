...
import com.sample.tools.config.MD5s;
...
public class OpsFile {
	 ...
	 * @throws ZipException
	 */
	public OpsEntry version() throws Exception {
		List<OpsEntry> oentrys = this.content(VER);
		return oentrys.size() > 0 ? oentrys.get(0) : null;
	}
	...
	private String version(File makeDir) throws IOException {
		...
		if (version.exists() && version.isFile()) {
			...
			if (_version != null) {
				...
				return Settings.me().getVersion().getName() + "." + vString.trim();
			}
		}
		return "0.1";
	}
	...
	private void compress(ZipOutputStream out, File parent, String path) throws IOException {
		if (parent.isDirectory()) {
			...
			for (File file : files) {
				this.compress(out, file, path + "/" + file.getName());
			}
