...
public abstract class PauseDetector {
	...
	protected final AtomicInteger threads = new AtomicInteger();
	 ...
	 * 
	 * @param pauseLengthNsec
	 *            pause length (in nanoseconds)
	 * @param pauseEndTimeNsec
	 *            pause end time (in nanoTime)
	 */
	...
	/**
	 * Add a {@link org.LatencyUtils.PauseDetectorListener} listener to be notified
	 * when pauses are detected. Listener will be added to the normal priority
	 * listeners list.
	 * 
	 * @param listener
	 *            Listener to add
	...
	/**
	 * Add a {@link org.LatencyUtils.PauseDetectorListener} listener to be notified
	 * when pauses are detected Listener will be added to either the normal priority
	 * or high priority listeners list,
	 * 
	 * @param listener
	 *            Listener to add
	 * @param isHighPriority
	 *            If true, listener will be added to high priority list. If false,
	 *            listener will be added to the normal priority list.
	 */
	public synchronized void addListener(PauseDetectorListener listener, boolean isHighPriority) {
		messages.add(new ChangeListenersRequest((isHighPriority ? ChangeListenersRequest.ChangeCommand.ADD_HIGH_PRIORITY
				: ChangeListenersRequest.ChangeCommand.ADD_NORMAL_PRIORITY), listener));
	}
	...
	private class PauseDetectorThread extends Thread {
		...
		public PauseDetectorThread() {
			super("Latency.Pause-" + threads.incrementAndGet());
		}
				...
				try {
					...
					if (message instanceof ChangeListenersRequest) {
						...
						for (PauseDetectorListener listener : highPriorityListeners) {
							listener.handlePauseEvent(pauseNotification.pauseLengthNsec,
									pauseNotification.pauseEndTimeNsec);
						}
	...
	static class ChangeListenersRequest {
		static enum ChangeCommand {
			ADD_HIGH_PRIORITY, ADD_NORMAL_PRIORITY, REMOVE
		};
