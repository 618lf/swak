...
/**
 * Provide an API for time-related service, such as getting the current time and
 * waiting for a given period of time. By default, these services are provided
 * by actual time services in the JDK (i.e. System.nanoTime(),
 * System.currentTimeMillis(), Thread.sleep(), and
 * java.util.concurrent.locks.LockSupport.parkNanos()). However, if the property
 * LatencyUtils.useActualTime is set to "false", TimeServers will only move the
 * notion of time in response to calls to the #setCurrentTime() method.
 *
 */
public class TimeServices {
	...
	// TODO: if we find this to be inefficient for some reason, replace with a timer
	// wheel or some such.
	...
	public static class ScheduledExecutor {
		...
		ScheduledExecutor() {
			if (useActualTime) {
				actualExecutor = new ScheduledThreadPoolExecutor(1, (r) -> {
					Thread t = new Thread(r);
					t.setDaemon(true);
					t.setName("Latency.TimeServices-1");
					return t;
				});
		...
		private class MyExecutorThread extends Thread {
			...
			public void run() {
				try {
					while (doRun) {
						synchronized (timeUpdateMonitor) {
							for (RunnableTaskEntry entry = taskEntries.peek(); ((entry != null)
									&& (entry.getStartTime() < currentTime)); entry = taskEntries.peek()) {
			...
			public void setNewStartTime(long timeNow) {
				if (period == 0) {
					throw new IllegalStateException(
							"should nto try to reschedule an entry that has no interval or rare");
				}
				if (!fixedRate) {
