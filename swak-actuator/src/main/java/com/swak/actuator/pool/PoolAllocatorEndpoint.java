 ...
 * @since 2.0.0
 */
@Endpoint(id = "poolAllocator")
public class PoolAllocatorEndpoint {
	...
	private Map<String, Object> metricsOfPoolArena(final PoolArenaMetric poolArenaMetric) {
		...
		/**
		 * Returns an unmodifiable {@link List} which holds
		 * {@link PoolChunkListMetric}s.
		 */
		 ...
		 * for tiny sub-pages.
		 */
		metrics.put("7_1_tinySubpages",
				metricsOfSubpages(poolArenaMetric.numTinySubpages(), poolArenaMetric.tinySubpages()));
		 ...
		 * for small sub-pages.
		 */
		metrics.put("8_1_smallSubpage",
				metricsOfSubpages(poolArenaMetric.numSmallSubpages(), poolArenaMetric.smallSubpages()));

		return metrics;
	}

	private Map<String, Object> metricsOfSubpages(final int count, final List<PoolSubpageMetric> subpages) {
		final Map<String, Object> metrics = new HashMap<>();
	...
	private Map<String, Object> metricsOfPoolSubpage(final PoolSubpageMetric subpageMetric) {
		final Map<String, Object> metrics = new HashMap<>();
		/**
		 * Return the number of maximal elements that can be allocated out of the
		 * sub-page.
		 */
	...
	private Map<String, Object> metricsOfChunkLists(final List<PoolChunkListMetric> chunkLists) {
		final Map<String, Object> metrics = new HashMap<>();
		int idx = 0;
	...
	private Map<String, Object> metricsOfPoolChunkList(final PoolChunkListMetric chunkListMetric) {
		final Map<String, Object> metrics = new HashMap<>();
	...
	private Map<String, Object> metricsOfPoolChunk(final PoolChunkMetric chunkMetric) {
		final Map<String, Object> metrics = new HashMap<>();
	...
	private long activeAllocationsInBytes(final PoolArenaMetric poolArenaMetric) {
		long totalBytes = 0;
		for (PoolChunkListMetric chunkListMetric : poolArenaMetric.chunkLists()) {
			// 包括 tinySubpages & smallSubpages 以及 Normal ( 单次分配 <= 16M 大小的分配) 及 Huge ( 单次分配
			// > 16M) 分配
	...
	private long activeAllocationsInBytes(final List<PoolSubpageMetric> subpages) {
		long totalBytes = 0;
	...
	private int activeAllocationsInBytes(final PoolSubpageMetric subpage) {
		return subpage.elementSize() * (subpage.maxNumElements() - subpage.numAvailable());
	}

	private long activeAllocationsInBytes(final PoolChunkListMetric chunkListMetric) {
		long totalBytes = 0;
		final Iterator<PoolChunkMetric> iter = chunkListMetric.iterator();
		while (iter.hasNext()) {
