 ...
 * @author root
 */
public class RedisLocalCache implements MessageListener, LocalCache<Object>, DisposableBean {

	private final String LOCAL_CACHE_TOPIC = "LOCAL_CACHE_TOPIC";
	private final EhcacheBase<String, byte[]> cache;
	private final String name;
	private final RedisAsyncPubSubCommands<byte[], byte[]> pubSuber;

	public RedisLocalCache(RedisService redisService, String name, EhcacheBase<String, byte[]> local) {
		this.name = name;
		this.cache = local;
		this.pubSuber = redisService.event();
	}
	...
	public void sendEvictCmd(Object key) {
		if (key != null) {
			...
			this.pubSuber.publish(SafeEncoder.encode(LOCAL_CACHE_TOPIC), cmd.toBuffers());
		}
	}
	 ...
	 */
	public void start() {
		this.pubSuber.subscribe(this, SafeEncoder.encode(LOCAL_CACHE_TOPIC));
	}
}