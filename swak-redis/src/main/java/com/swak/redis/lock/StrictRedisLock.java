...
/**
 * 严格的 redis 的锁 需要使用单独的线程池来执行，根据锁的个数来决定线程池的大小 不建議單獨使用<br>
 * redis锁的最大问题是：1. 无法预估时间，时间少了则有可能还没执行结束。<br>
 * 2. 时间长了，有可能持有锁之后挂掉了，其他线程执行等待超时<br>
 * 3. 需要有一个守护线程来不停的更新时间<br>
 * 
 * @author lifeng
 */
public class StrictRedisLock implements Lock {
	 ...
	 * 锁超时时间， 需要定时任务不停的更新时间
	 */
	private long lockTimeout = 10 * 1000; // 10s
	 ...
	 * 
	 * @param name
	 * @param executor 用于延长执行时间
	 */
	public StrictRedisLock(String name, ExecutorService executor) {
		this.name = new StringBuilder("Lock:").append(name).toString();
	...
	public boolean tryLock() {
		...
		return RedisUtils.sync(jedis -> {
			...
			return jedis.setnx(lockKey, value);
		});
	}
