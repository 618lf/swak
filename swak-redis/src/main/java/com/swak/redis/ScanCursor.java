...
public abstract class ScanCursor<T> implements Cursor<T> {
	...
	public final ScanCursor<T> open() {
		...
		if (!isReady()) {
			throw new AccessDeniedException("Cursor already " + state + ". Cannot (re)open it.");
		}
	...
	private void assertCursorIsOpen() {
		...
		if (isReady() || isClosed()) {
			throw new AccessDeniedException("Cannot access closed cursor. Did you forget to call open()?");
		}
	}
