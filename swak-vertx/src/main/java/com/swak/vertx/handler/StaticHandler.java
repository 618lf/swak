...
public class StaticHandler implements io.vertx.ext.web.handler.StaticHandler {
	...
	private void writeCacheHeaders(HttpServerRequest request, FileProps props) {
		...
		headers.set("date", GmtDateKit.format());
	}
	...
	private synchronized void isFileExisting(RoutingContext context, String file,
					...
					if (contentType != null) {
						if (contentType.startsWith("text")) {
							request.response().putHeader("Content-Type",
									contentType + ";charset=" + defaultContentEncoding);
						} else {
							...
							if (!dependency.isNoPush()) {
								...
								getFileProps(context, dep, filePropsAsyncResult -> {
									if (filePropsAsyncResult.succeeded()) {
										...
										response.push(HttpMethod.GET, "/" + dependency.getFilePath(),
												pushAsyncResult -> {
													if (pushAsyncResult.succeeded()) {
														HttpServerResponse res = pushAsyncResult.result();
														final String depContentType = MimeMapping
																.getMimeTypeForExtension(file);
														if (depContentType != null) {
															if (depContentType.startsWith("text")) {
																res.putHeader("Content-Type", contentType + ";charset="
	...
	private final class CacheEntry {
		...
		// return true if there are conditional headers present and they match what is
		// in the entry
