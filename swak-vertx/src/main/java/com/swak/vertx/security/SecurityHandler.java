...
public class SecurityHandler implements io.vertx.core.Handler<RoutingContext>, InitializingBean {
    ...
    public void handle(RoutingContext context) {
        this.securityManager.createSubject(context).thenCompose(subject -> this.doFilter(context, subject)).whenComplete((v, e) -> {
            if (v) {
                context.next();
            }
        });
    ...
    private CompletionStage<Boolean> doFilter(RoutingContext context, Subject subject) {
        ...
        // 找到一个最匹配的
        Optional<List<Handler>> optional = chains.keySet().stream().filter(s -> StringUtils.startsWith(url, s)).findFirst().map(s -> {
