...
public class MainVerticle extends AbstractVerticle {
    ...
    private List<Future<String>> startHttp(ServiceBean service) {
        ...
        for (int i = 1; i <= intstances; i++) {
            futures.add(Future.future(s -> vertx.deployVerticle(new HttpServerVerticle(routerHandler, properties.getHost(), properties.getPort()),
                    options, s)));
        }
        return futures;
    }
	...
	private List<Future<String>> startService(ServiceBean service) {
        ...
        int intstances = getDeploymentIntstances(service);
        for (int i = 1; i <= intstances; i++) {
            futures.add(Future.future(s -> vertx.deployVerticle(new ServiceVerticle(this.getProxyService(service), service.getServiceType()),
					options, s)));
        }
        return futures;
    }
