...
abstract class ContextImpl extends AbstractContext {
	...
	/**
	 * Execute the {@code task} disabling the thread-local association for the
	 * duration of the execution. {@link Vertx#currentContext()} will return
	 * {@code null},
	 * 
	...
	protected TaskQueue orderedTasks;

	protected ContextImpl(VertxInternal vertx, VertxTracer<?, ?> tracer, WorkerPool internalBlockingPool,
	...
	protected ContextImpl(VertxInternal vertx, VertxTracer<?, ?> tracer, EventLoop eventLoop,
			WorkerPool internalBlockingPool, WorkerPool workerPool, Deployment deployment, ClassLoader tccl) {
