package com.veni.tools.view.ticker;
...
class TickerColumnManager {
    ...
    void setText(char[] text) {
        ...
        for (int i = 0; i < mRxTickerColumns.size(); ) {
            final TickerColumn rxTickerColumn = mRxTickerColumns.get(i);
        ...
        for (int i = 0; i < actions.length; i++) {
            switch (actions[i]) {
                ...
                case LevenshteinUtils.ACTION_DELETE:
                    mRxTickerColumns.get(columnIndex).setTargetChar(TickerUtils.EMPTY_CHAR);
                    columnIndex++;
                    break;
                default:
    ...
    void onAnimationEnd() {
        for (int i = 0, size = mRxTickerColumns.size(); i < size; i++) {
            final TickerColumn column = mRxTickerColumns.get(i);
