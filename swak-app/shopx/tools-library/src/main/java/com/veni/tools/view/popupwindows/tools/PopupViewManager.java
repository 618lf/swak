...
public class PopupViewManager {
    ...
    private static final String TAG = PopupViewManager.class.getSimpleName();
    ...
    public PopupViewManager(TipListener listener) {
        mAnimationDuration = DEFAULT_ANIM_DURATION;
        mListener = listener;
    }

    public View show(PopupView rxPopupView) {
        ...
        PopAnimationTool.popup(tipView, mAnimationDuration).start();

        return tipView;
    }

    private View create(PopupView rxPopupView) {
        ...
        PopupViewBackgroundConstructor.setBackground(tipView, rxPopupView);

        // add tip to root layout
        rxPopupView.getRootView().addView(tipView);

        // find where to position the tool tip
        Point p = PopupViewCoordinatesFinder.getCoordinates(tipView, rxPopupView);
    ...
    private void setTipViewElevation(TextView tipView, PopupView rxPopupView) {
    ...
    private void switchToolTipSidePosition(PopupView rxPopupView) {
        if (rxPopupView.positionedLeftTo()) {
            rxPopupView.setPosition(PopupView.POSITION_RIGHT_TO);
        } else if (rxPopupView.positionedRightTo()) {
            rxPopupView.setPosition(PopupView.POSITION_LEFT_TO);
        }
    }
    ...
    private void animateDismiss(final View view, final boolean byUser) {
        PopAnimationTool.popout(view, mAnimationDuration, new AnimatorListenerAdapter() {
            @Override
