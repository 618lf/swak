...
public class MethodCache {
	...
	public static class MethodMeta {
		...
		private Class<?>[] getParameterTypes(Map<TypeVariable<?>, Type> paramVariablesMappers) {
			...
			if (paramVariablesMappers != null && actualTypes.length > 0 && parameterTypes != null
				...
				for (int i = 0; i < parameterTypes.length; i++) {
					if (parameterTypes[i] instanceof ParameterizedType) {
						...
						if (actualType instanceof Class) {
							...
							actualTypes[i] = this.getNestActualType(actualType, paramVariablesMappers);
						} else {
							if (logger.isDebugEnabled()) {
						...
						actualTypes[i] = this.getNestActualType(parameterTypes[i], paramVariablesMappers);
					}
				}
			}
			return actualTypes;
		 ...
		 */
		public MethodMetrics applyMetrics(MetricsFactory metricsFactory, String metricName) {
			if (metricsFactory != null) {
				this.metrics = metricsFactory.createMethodMetrics(this.method, metricName);
			}
			return this.metrics;
		}
