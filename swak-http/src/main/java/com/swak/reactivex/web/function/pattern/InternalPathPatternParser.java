...
import com.swak.reactivex.web.function.pattern.PatternParseException.PatternMessage;
...
public class InternalPathPatternParser {
	 ...
	 * 
	 * @param parentParser
	 *            reference back to the stateless, public parser
	 */
	...
	/**
	 * After processing a separator, a quick peek whether it is followed by ** (and
	 * only ** before the end of the pattern or the next separator)
	 */
	...
	/**
	 * @param newPathElement
	 *            the new path element to add to the chain being built
	 */
		...
		if (this.variableCaptureCount > 0) {
			if (this.variableCaptureCount == 1 && this.pathElementStart == this.variableCaptureStart
				...
				if (this.isCaptureTheRestVariable) {
					// It is {*....}
					newPE = new CaptureTheRestPathElement(this.pathElementStart, getPathElementText(),
							this.parser.getSeparator());
				} else {
	...
	/**
	 * For a path element representing a captured variable, locate the constraint
	 * pattern. Assumes there is a constraint pattern.
	 * 
	 * @param data
	 *            a complete path expression, e.g. /aaa/bbb/{ccc:...}
	 * @param offset
	 *            the start of the capture pattern of interest
	 * @return the index of the character after the ':' within the pattern
	 *         expression relative to the start of the whole expression
	 */
	...
	/**
	 * Reset all the flags and position markers computed during path element
	 * processing.
	 */
	...
	/**
	 * Record a new captured variable. If it clashes with an existing one then
	 * report an error.
	 */
