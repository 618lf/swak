...
public class ImAnnotationBeanProcessor implements EnvironmentAware, BeanDefinitionRegistryPostProcessor {
	...
	private String generateServiceBeanName(Class<?> interfaceClass, Method method) {
		return new StringBuilder("@ImApi").append(" ").append(interfaceClass.getName()).append(".")
	...
	private AbstractBeanDefinition registryBeans(Class<?> beanClass, String beanName, Method method) {
		BeanDefinitionBuilder builder = rootBeanDefinition(ImBean.class);
	...
	protected boolean isController(Class<?> clazz) {
		return (clazz.isAnnotationPresent(ImApi.class) || clazz.isAnnotationPresent(ImService.class));
	}
