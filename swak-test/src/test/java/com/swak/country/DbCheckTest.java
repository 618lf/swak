...
//		countrys = PickerCheckTest.countrys();
//		for (Country country : countrys) {
//			Oecd oecd = oecdService.get(country.address);
//			if (oecd == null) {
//				System.out.println(country);
...
class OecdService {
	...
	public Oecd get(String name) {
		...
		param.put("NAME", name);
		List<Oecd> datas = jdbcTemplate.query("SELECT NAME FROM NONR_AGREEMENT_COUNTRY WHERE NAME=:NAME", param,
				getRowMapper());
	...
	public List<Oecd> getAll() {
		Map<String, Object> param = Maps.newHashMap();
		List<Oecd> datas = jdbcTemplate.query("SELECT NAME FROM NONR_AGREEMENT_COUNTRY", param, getRowMapper());
		return datas;
	}
